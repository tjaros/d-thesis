...
\begin{itemize}
    \item \textbf{Direct Anonymous Attestation} \cite{daaSpec},
...
The TPMs contain only specific widely used cryptographic primitives. These primitives may be used independently by calling the corresponding command defined in the TPM specification, or as building blocks of more complex cryptographic protocols and schemes. 
...
\begin{itemize}
    \item \textbf{TPM2\_Create} or \textbf{TPM2\_CreatePrimary} which are used for the generation of asymmetric key pairs the difference between the two is that for the latter one, the key generation uses a special secret key stored in the TPM as a seed.
    \item \textbf{TPM2\_Load} which loads the key into the TPM. For the TPM to be able to work with the key it must be loaded, because the previously created key may not be stored inside of the TPM.
    \item \textbf{TPM2\_ActivateCredential} which is used to certify the created key. The public portion of the generated key and public portion of TPM's endorsement key is sent by the platform associated with TPM to the Certification Authority which checks the validity of TPM's endorsement key and creates a certificate for the generated key and a new symmetric key. Then it sends both back to the platform.
    \item \textbf{TPM2\_Commit} is a first, but optional part of the signing process defined in \cite{chen2013flexible}. The TPM generates commitment and returns associated values and a reference to the commitment value. Commitments are fundamentally used in most cryptographic protocols, for more information about commitments see~\cite{damgaard1998commitment}. 
    \item \textbf{TPM2\_Sign} which is used to sign a given message, specifically its hash. If a TPM2\_Commit command was called before and the signature algorithm uses commitments, then the commitment is retrieved using the reference given by the TPM2\_Commit command.
\end{itemize}
Chen and Li \cite{chen2013flexible} also discuss the possible applications of this signature primitive for the implementation of protocols and schemes such as Direct Anonymous Attestation~\cite{daaSpec}, U-Prove~\cite{uproveSpec}, or more conventional signature schemes such as Schnorr signatures~\cite{schnorrSpec}. However, they only provide a high-level theoretical definition, but only little of implementation-specific details. 
\section{Research}
